...
export function LibraryManagePage() {
  ...
  const createPhaseMutation = useMutation({
    mutationFn: (name: string) => createLibraryAuditPhase({ name }),
    onSuccess: () => {
  ...
  const updatePhaseMutation = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) => updateLibraryAuditPhase(id, { name }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["auditPhases"] });
                        ...
                        <button
                          ...
                          onClick={async () => {
                            const ok = await confirm({
                              ...
                              message: `Tem certeza que deseja excluir a disciplina "${discipline.name}"? Ela será desativada e não aparecerá em novas auditorias.`,
                ...
                <form
                  onSubmit={(e) => {
                    ...
                    if (newPhaseName.trim()) {
                      createPhaseMutation.mutate(newPhaseName.trim());
                    }
                  }}
                  className="flex items-center gap-4"
                >
                ...
                {editingPhaseId === phase.id ? (
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      if (editName.trim()) {
                        updatePhaseMutation.mutate({ id: phase.id, name: editName.trim() });
                      }
                    }}
                    className="flex items-center gap-4"
                  >
                        ...
                        <button
                          ...
                          onClick={async () => {
                            const ok = await confirm({
                              ...
                              message: `Tem certeza que deseja excluir a categoria "${category.name}"? Ela será desativada e não aparecerá em novas auditorias.`,
                              confirmLabel: "Excluir",
                              variant: "danger",
                            });
                            if (ok) deleteCategoryMutation.mutate(category.id);
                          }}
              ...
              <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-4 py-2 text-sm font-medium text-[hsl(var(--foreground))] hover:opacity-90"
                  >
                    <NavArrowIcon direction="back" className="h-4 w-4" />
                    Voltar
                  </button>
                    ...
                    {canEdit && (
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            const ok = await confirm({
                              title: "Excluir item de verificação",
                              message: "Tem certeza que deseja excluir este item de verificação? Ele será desativado e não aparecerá em novas auditorias.",
                              confirmLabel: "Excluir",
                              variant: "danger",
                            });
                                      ...
                                      onChange={(e) => {
                                        ...
                                        updateCategoryPhases(category.id, Array.from(newPhases))
                                          ...
                                          .catch((error) => {
                                            ...
                                            toast.error(error instanceof Error ? error.message : "Erro ao atualizar aplicabilidade");
                                          })
                                          .finally(() => {
