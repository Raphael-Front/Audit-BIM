                  ...
                  {items.map((i) => (
                    <div key={i.id} className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-sm">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className={`rounded-full px-3 py-1 text-xs font-medium shrink-0 ${i.status === "CONFORMING" ? "bg-emerald-600 text-white" : i.status === "NONCONFORMING" ? "bg-red-600 text-white" : i.status === "NA" ? "bg-zinc-400 text-black" : "bg-[hsl(var(--muted))] text-[hsl(var(--macro))]"}`}>{getStatusLabel(i.status)}</span>
                      </div>
