...
export default function AuditDetailPage() {
  ...
  return (
    <div>
      <Link to="/audits" className="mb-4 inline-block text-sm text-[hsl(var(--accent))] hover:underline">← Voltar à lista</Link>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {canCancelAudit && (
            <button
              ...
              className="rounded border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-4 py-2 text-sm font-medium hover:bg-[hsl(var(--muted))] disabled:opacity-50"
            >
      ...
      <h2 className="mb-2 text-lg font-medium text-[hsl(var(--foreground))]">Itens</h2>
      <ul className="space-y-2">
        {(items as AuditItemRow[]).map((item) => (
          <li key={item.id} className="rounded border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="font-medium text-[hsl(var(--foreground))]">{itemDescription(item)}</p>
                <p className="text-xs text-[hsl(var(--muted-foreground))]">{itemDiscipline(item)}</p>
              </div>
              <span className="rounded px-2 py-0.5 text-xs text-[hsl(var(--muted-foreground))]">
                {itemStatusLabels[item.status] ?? item.status}
              </span>
            </div>
            {item.evidenceText && <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{item.evidenceText}</p>}
            {item.construflowRef && <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Construflow: {item.construflowRef}</p>}
          </li>
        ))}
      </ul>
    </div>
