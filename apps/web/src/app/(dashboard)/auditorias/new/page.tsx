...
export default function NewAuditoriaPage() {
  ...
  return (
    <Container>
      <div className="mb-6">
        <Link href="/auditorias" className="text-sm text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]">← Auditorias</Link>
      </div>
      <h1 className="text-3xl font-semibold tracking-tight text-[hsl(var(--foreground))]">Nova auditoria</h1>
      <form onSubmit={handleSubmit} className="mt-8 max-w-md space-y-4">
        ...
        <div>
          <label htmlFor="auditPhase" className="block text-sm font-medium text-[hsl(var(--foreground))]">Fase de auditoria (PL, LO, etc.) *</label>
          <select id="auditPhase" value={auditPhaseId} onChange={(e) => setAuditPhaseId(e.target.value)} required className="mt-1 w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--background))] px-3 py-2">
            <option value="">Selecione</option>
            {auditPhases.map((a) => (
