-- sync_obra_score: usa fato_auditoria_itens como fallback quando tbl_scores_calculados
-- não tem a auditoria (ex: auditorias sem sync, ou com score excluído).
-- Corrige obras como TRIP WORLD HOME e TERRA UNNA que ficavam zeradas.
...
BEGIN
  ...
  -- CTE: score por auditoria (tbl_scores_calculados OU fato_auditoria_itens)
  WITH aud_base AS (
